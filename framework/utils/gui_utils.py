...
from framework.utils.string_utils import contains_regex_characters, strip_anchor_chars
...
def mouse_button_down(button):
    if not _isWin():
        ...
        pyautogui.mouseDown(button=button, duration=0.1)
...
def mouse_button_up(button):
    if not _isWin():
        ...
        pyautogui.mouseUp(button=button, duration=0.1)


def mouse_button_down_hold(button, duration):
    pyautogui.mouseDown(button=button, duration=duration)


def mouse_button_up_hold(button):
    pyautogui.mouseUp(button=button, duration=1)
    time.sleep(1)
