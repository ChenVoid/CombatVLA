...
from framework.utils.json_utils import load_json
from framework.utils import Singleton
from framework.utils.file_utils import assemble_project_path, get_project_root
from framework.utils.dict_utils import kget
from framework.utils.gui_utils import get_screen_size, get_active_window, get_named_windows, get_named_windows_fallback
...
class Config(metaclass=Singleton):
    ...
    load_list = os.listdir(run_path)
    assert not len(load_list) > 1
    if load_list:
        load_path = load_list[0]
    else:
        load_path = ""
    ...
    def _set_dirs(self) -> None:
        ...
        if self.load_path:
            self.work_dir = assemble_project_path(os.path.join(self.work_dir, self.load_path))
        else:
